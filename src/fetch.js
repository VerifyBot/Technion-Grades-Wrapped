...
function selectAndExtractCourses(htmlContent) {
  const parser = new DOMParser();
  const doc = parser.parseFromString(htmlContent, 'text/html');

  // Find all available semesters in the sidebar
  const semesterNodes = doc.querySelectorAll('li.sem');
  const semesters = [];

  semesterNodes.forEach((node) => {
    const titleElement = node.querySelector('a');
    const title = titleElement.textContent.replace(/\s+/g, ' ').trim();
    semesters.push({ title, node });
  });

  // Build the prompt text for the user
  let promptText = "Enter the number of the semester you want:\n";
  semesters.forEach((sem, i) => {
    promptText += `(${i + 1}) ${sem.title}\n`;
  });

  // Prompt the user
  const userChoice = prompt(promptText);
  let selectedIndex = parseInt(userChoice, 10) - 1;

  // Validate the choice (fallback to 0 if null, empty, or invalid)
  if (isNaN(selectedIndex) || selectedIndex < 0 || selectedIndex >= semesters.length) {
    console.warn("Invalid or no choice made. Defaulting to the latest semester (1).");
    selectedIndex = 0;
  }

  // Extract courses from the chosen semester
  const selectedSemesterNode = semesters[selectedIndex].node;
  const courseLinks = selectedSemesterNode.querySelectorAll('.sub-menu a.ga-course');
  const courses = [];
  ...
  alert('Fetching grades, please stand by (~10 seconds)');
  ...
  for (let course of lastSemesterCourses) {
    ...
    for (const row of gradeRows) {
      ...
      let date = cells[6].innerHTML.replace(/<br[^>]*>/i, ' ').trim().replace('&nbsp;', '')
        || cells[7].innerHTML.replace(/<br[^>]*>/i, ' ').trim().replace('&nbsp;', '');
...
const normalizeExamData = (data) => {
  return data.map(course => {
    ...
    if (updatedCourse.moedA && updatedCourse.moedA.date) {
      updatedCourse.moedA = { ...updatedCourse.moedA, date: normalizeDate(updatedCourse.moedA.date) };
    }
    if (updatedCourse.moedB && updatedCourse.moedB.date) {
      updatedCourse.moedB = { ...updatedCourse.moedB, date: normalizeDate(updatedCourse.moedB.date) };
    }
...
(async () => {
  try {
    ...
    const selectedCourses = selectAndExtractCourses(content);
    const rawData = await processCourseData(selectedCourses);
    const normalizedData = normalizeExamData(rawData);
    const finalData = transformGrades(normalizedData);
    
    downloadDataAsJSON(finalData);
